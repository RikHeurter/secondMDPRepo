...
//      8. Check which of the two versions of line 447 I want   ASAP
...
class MarkovDP{
    ...
    public:
        ...
        MarkovDP(int dimp, vector<float> ratesp, vector<float> speedsp, vector<int> coresp, int maxsizep, float arrivalratep, float departureratep, int cutoffp, float epsp){
            // cout << "A" << endl;
    ...
    private:
        ...
        */
        void build_markov_chain(){
            ...
            valuefunc = vector<float> (actualdims, 0);
        ...
        */
        int binsearch_partitions(vector<int> goal){
            ...
            while (lower != heigher - 1 && lower != heigher){
                mid = (lower + heigher)/2;
                // cout << "mid: " << mid << "heigher: " << heigher << "lower: " << lower << endl;
        ...
        */
        vector<vector<int>> index_to_partitions(int index){
            ...
            for (int i = 1; i < goal_indices.size(); i++){
                ...
                for (int i = 0; i < partitionsizes.size(); i++){
                    if (partitionsizes[i] > goal_index-1){
                        num_cores_working = i;
                        break;
                    }
                }
        ...
        float SAStep(){
            vector<float> newvaluefunc(valuefunc.size(), 0);
            ...
            for (int i = 0; i < valuefunc.size(); i++){
                if (i % 10000 == 0){
                    cout << "Done: " << i/(float)valuefunc.size() << "%" << endl;
                }
                // cout << i << endl;
...
int main(){
    ...
    MarkovDP alder_lake = MarkovDP(2, {0.8, 0.8}, {3.6, 2.7}, {8, 4}, 100, 25, 1, 100, 1);
