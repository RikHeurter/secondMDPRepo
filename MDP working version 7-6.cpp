...
class MarkovDP{
    ...
    private:
        ...
        */
        ...
        double Simulat(vector<vector<int>> allocatedjobs, int cur_waiting){
            return 0;
        }

        // vector<vector<int>> index_to_allocation (int index){
        ...
        double SAStepversion2(){
            ...
            for (int i = 0; i < valuefunc.size(); i++){
                ...
                { // Just some scoping :)
                    ...
                    for (int k = 0; k < dim; k++){
                        if (possible_allocations[k] > 0){
                            // max_index += (1+partitionsizes[possible_allocations[k]-1])*cur_dim;
                    ...
                    for (int j = 0; j < cur_dim; j++){
                        ...
                        // cout << "Number of partitions: " << allocations.size() << endl;
                        // cout << "Size of actual allocations: " << non_updated_allocation.size() << endl;
                        ...
                        if (after_alloc_waiting_jobs < cutoff){
                            ...
                            //     for (vector<int> part : non_updated_allocation){
                            //         cout << "Partition:";
                            //         for (int val : part){
                            //             cout << ", " << val;
                            //         }
                            //         cout << endl;
                            //     }
                            //     cout << "Waiting jobs: " << after_alloc_waiting_jobs << endl;
                            //     cout << "Cur_free_cores: " << endl;
                            //     for (int free_cores : cur_free_cores){
                            //         cout << ", " << free_cores;
                            //     }
                            //     cout << endl;
                            //     cout << "Pre-choice partition: " << endl;
                            //     for (vector<int> part : actualpartitions){
                            //         cout << "Partition:";
                            //         for (int val : part){
                            //             cout << ", " << val;
                            //         }
                ...
                // cout << "E" << endl;
                    // allocations[0]++;
                    // for (int k = 0; k < dim; k++){
                    //     if (allocations[k] > cur_free_cores[k] && k + 1 != cur_free_cores.size()-1){
                    //         allocations[k] = 0;
                    //         allocations[k+1]++;
                    //     }
                    // }

                    // for (vector<int> partition : partitions[allocated_cores]){
                    //     vector<int> uncollapsepartition;
                    //     for (int k = 0; k < partition.size(); k++){
                    //         for (int l = 0; l < partition[k]; l++){
                    //             uncollapsepartition.push_back(k);
                    //         }
                    //     }
                    //     vector<int> allocations (0,uncollapsepartition.size());

                    // }
                    // vector<int> allocations (dim-1, 0);
                    // while (true){
                    //     vector<vector<int>> copied_partition (actualpartitions);
                    //     for (int k = 0; k < allocations.size(); k++){
                    //         while (true){
                    //             for (vector<int> allocation_partition : partitions[k]){
                    //                 vector<int> curpart (copied_partition[k]);
                    //                 if (allocation_partition.size() > curpart.size()){
                    //                     for (int j = 0; j < allocation_partition.size() - curpart.size(); j++){
                    //                         curpart.push_back(0);
                    //                     }
                    //                 }
                    //             }
