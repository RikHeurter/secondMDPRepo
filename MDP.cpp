...
//      9. Fix this epsilon only growing bug using arbitrary precision floats. DONE
//      4. Add some fancy way to output vals;
//      5. Create plots in Matplotlib.
//      6. Add a way to input percentages.                      Not usefull
//      7. Some more clean-up                                   DONE
...
class MarkovDP{
    ...
    public:
        ...
        void MonteCarlo(int iters, bool load_actions = false, string actions_file = "action_data.txt", string save_file_every_state = "whole_states_data.txt", string save_file_specific_state = "compressed_states_data.txt"){
                    ...
                    if (actions[node]==0){
                        ...
                        for (int dimmy = 0; dimmy < dim; dimmy++){
                            for (int k = 0; k < actualpartitions[dimmy].size(); k++){ // Why does this need to be actualpartitions.size()???
                                ...
                                assert(copied_partition[dimmy][k] >= 0);
                                ...
                                new_probs[calc_index(copied_partition, waiting_jobs)] += (actualpartitions[dimmy][k] *
                                            ...
                                            probs[node];
                                sum_rates += actualpartitions[dimmy][k] * (calcAmdahlsLaw(speeds[dimmy], rates[dimmy], k))/uniformization_constant;
                            }
                        }
                        ...
                        for (int k = 0; k < allocations.size(); k++){
                            ...
                            if (allocationss.size() != 1 || allocationss[0] != 0){
                                ...
                                for (int l = 0; l < allocationss.size(); l++){
                                    ...
                                    sum_allocs += allocationss[l];
                                    total_alloacted_k += l*allocationss[l];
                                }
                            }
                            if (sum_allocs > waiting_jobs || total_alloacted_k > cores[k]){
                                assert(0 == 1), "error in monte carlo, impossible action chosen";
                                cout << "Error!" << endl;
                                break;
                            }
                        }
                        ...
                        for (int dimmy = 0; dimmy < dim; dimmy++){
                            for (int k = 0; k < non_updated_allocation[dimmy].size(); k++){ // Why does this need to be actualpartitions.size()???
                                ...
                                // cout << copied_partition[dimmy][k] << endl;
                                copied_partition[dimmy][k] -= 1;
                                // cout << copied_partition[dimmy][k] << endl;
                                double addition = (non_updated_allocation[dimmy][k] *
                                            ...
                                            uniformization_constant);
                                // if (cur_jobs < 20){
                                //     cout << "Added: " << addition << ". By allocating: " << non_updated_allocation[dimmy][k] << " of type: " << dimmy << " with parallelisation_level " << k << ", yielding total speed-up of: " << calcAmdahlsLaw(speeds[dimmy], rates[dimmy], k) << endl;
                                // }
                        ...
                        if (after_alloc_waiting_jobs < cutoff){
                            ...
                            // if (waiting_jobs < 10){
                            //     cout << "sum_rates: " << sum_rates << ", with currently " << waiting_jobs << " jobs " << "at index: " << node << endl;
                            // }
                            // if (cur_jobs < 20){
                            //     cout << "sum_rates (without gaining jobs): " << sum_rates << ", with currently " << waiting_jobs << " jobs " << "at index: " << node << endl;
                            //     assert (sum_rates < 0.5);
                            // }
                            sum_rates += arrivalrate/uniformization_constant;
                        }

                        assert(-0.1 < sum_rates &&sum_rates <= 1.1);
            ...
            ofstream whole_data (save_file_every_state);
            ofstream compressed_data (save_file_specific_state);
            cout << "cut off: " << cutoff << endl;
            vector<double> compressed_probs (cutoff+15,0);
            cout << "compressed_probs size: " << compressed_probs.size() << endl;
    ...
    private:
        ...
        */
        void build_markov_chain(){
            ...
            cout << "Setting uniformization_constant at: " << uniformization_constant << endl;
...
int main(){
    ...
    // MarkovDP alder_lake = MarkovDP(2, {0.8, 0.8}, {3.6, 2.7}, {8, 4}, 100, 25, 1, 100, 25);
    // MarkovDP alder_lake = MarkovDP(2, {0.8, 0.8}, {3.6, 2.7}, {1, 1}, 100, 25, 1, 100, 0.0001);
    // alder_lake.SA_general();
    // alder_lake.save_actions("longer_runtime.txt");
    // alder_lake.MonteCarlo(1000, false, "action_data.txt", "whole_states_data_long_run.txt", "compressed_states_data_long_run.txt");
    // for (int i = 39; i < 39.6; i++){
    {
        float i = 55;
        cout << "Currently optimising state: " << i << endl;
        string action_file_name = "data/action_data_arrival_" + to_string(i) + ".csv";
        string whole_file_name = "data/state_data_arrivalv2_" + to_string(i) + ".csv";
        string compressed_file_name = "data/compressed_data_arrivalv2_" + to_string(i) + ".csv";
        cout << action_file_name << endl;
        MarkovDP alder_lake = MarkovDP(2, {0.8, 0.8}, {3.6, 2.7}, {8, 4}, 100, i, 1, 100, 0.0001);
        // MarkovDP alder_lake = MarkovDP(2, {0.8, 0.8}, {3.6, 2.7}, {1, 1}, 100, (float)i/8.f, 1, 100, 0.0001);
        alder_lake.SA_general();
        alder_lake.save_actions(action_file_name);
        alder_lake.MonteCarlo(101, false, action_file_name, whole_file_name, compressed_file_name);
    }
    // cout << 4*calcAmdahlsLaw(3.6, 0.8, 1) + 1*calcAmdahlsLaw(3.6, 0.8, 4) + 2*calcAmdahlsLaw(2.7, 0.8, 1) + calcAmdahlsLaw(2.7, 0.8,2) << endl;
}
